...
- **Network Degradation and Background Retries:** Both the Agent App and the Customer Portal use hand-written fetch API clients for server state and data fetching. On a transient network failure the UI is designed to avoid a catastrophic crash screen and instead surface a non-intrusive, persistent banner at the top of the viewport: "Connection unstable. Automatically retrying..." — preserving user calm and session integrity.
