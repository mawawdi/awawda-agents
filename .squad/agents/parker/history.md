...
- T17 revision (post-#42 rejection): implemented runtime customer portal routes (`/m/:token` activation and `/order` composition UI), wired real API fetch flow + mobile/weak-network states, and added route-level vitest coverage plus Playwright verification rerun.
