...
- T03 established `@awawda/shared-types` with versioned `v1` Zod contracts for customer/session/order flows, plus validation tests to enforce API/frontend schema consistency.
...
- T12 PR #40 conflict refresh: rebased `parker/issue-12-session-activation` onto latest `main` (no semantic drift in activation/portal lifecycle code), regenerated Prisma client, and revalidated API quality gates (`pnpm --filter @awawda/api lint`, `test` 41/41, `build`) before merge retry.
...
- 2026-04-07 orders repository typing hotfix: replaced `idempotencyKey` delegate field accesses in `apps/api/src/orders/orders.repository.ts` with parameterized SQL (`$queryRaw/$executeRaw`) for reserve/replay/finalize paths, added strict JSON replay-body parsing guards, and revalidated repo gates (`pnpm lint`, `pnpm test`, `pnpm build`) plus per-app test commands.
